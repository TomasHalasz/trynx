...
var LiveForm = {
	options: {
		...
		errorMessageClass: 'popover',			    // CSS class for an error message
		...
		arrowClass: 'arrow'				    //T. Halasz arrow class
	},
...
LiveForm.addError = function(el, message) {
	...
	//if ($('#'+el.form.id).hasClass('showArrow')) //T. Halasz show arrow if is enabled by class
	//{
	  //  message = '<div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content">' + message + '</div>';
	//}
	var error = this.getMessageElement(el);
	//error.innerHTML = message;

	//T.H. popover must have delay before showing with new content

	var oldMess = ($("#" + el.id).next().find('.popover-content').html());
	if (oldMess != message)
	{
	    $("#" + el.id).popover('destroy');	    
	    timeoutID = window.setTimeout(function(){
		$("#" + el.id).popover({content: message,placement: 'bottom', toggle: 'popover'});	    
		$("#" + el.id).popover('show');	
	    }, 250);
	}
	//$("#" + el.id).next().find('.popover-content').html(message);
...
LiveForm.getMessageElement = function(el) {
	...
	if (!error) {
		...
		error.id = id;
		//var myArrow = document.createTextNode('<div class="arrow"></div>');
		//error.appendChild(myArrow);					
...
/**
 * Validates whole form version 2 called separately.
 */
Nette.validateForm2 = function(sender) {
	var form = sender.form || sender, scope = false;
	LiveForm.forms[form.id].hasError = false;
	if (form['nette-submittedBy'] && form['nette-submittedBy'].getAttribute('formnovalidate') !== null) {
		var scopeArr = Nette.parseJSON(form['nette-submittedBy'].getAttribute('data-nette-validation-scope'));
	...
	for (i = 0; i < form.elements.length; i++) {
		...
		if (!Nette.validateControl(elem)) {
			//return false;
			//$('#'+elem.id).parent().children().find('.arrow').show();
		}
	}
	return true;
};
...
Nette.initForm = function(form) {
	...
	if ($('#'+form.id).hasClass('noValidate'))
	{
	    return;
	}
	...
	Nette.addEvent(form, 'submit', function(e) {
		...
		//alert($('#'+form.id).hasClass('noLoader'));
		if (!$('#'+form.id).hasClass('noLoader'))
		{
		    $("#loading").show(); //T.Halasz Show ajax loader in case of succes validation
		}
	});
