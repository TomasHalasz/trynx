...
class SyncInvoicepayPresenter  extends \App\APIModule\Presenters\SyncPresenter{
    ...
    */
	...
	public function actionGet()
	{
		...
		$xml = Array2XML::createXML('cl_invoice_payments', $tmpDataArr);
		echo $xml->saveXML();
		$this->terminate();		
	}
	...
	public function actionGetNew()
	{
		...
		$tmpData = $this->DataManager->findAllTotal()->where(array('cl_invoice_payments.cl_company_id' => $this->cl_company_id))->
								where('cl_invoice_payments.changed >= ? OR cl_invoice_payments.created >= ?', $this->sync_last, $this->sync_last);
		if (!empty($this->dataxml))
		{
			$tmpData = $tmpData->where('cl_invoice_payments.id NOT IN(?)', $this->dataxml);
		}
		
		$tmpData = $tmpData->select('cl_invoice_payments.*');
		
		$tmpDataArr = array();
		foreach($tmpData as $key => $one)
		{
			$cl_partners_book = $one->toArray();
			$tmpDataArr['cl_invoice_payments'][] = $cl_partners_book;
