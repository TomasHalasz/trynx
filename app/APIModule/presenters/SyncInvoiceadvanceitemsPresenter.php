...
class SyncInvoiceadvanceitemsPresenter  extends \App\APIModule\Presenters\SyncPresenter{
    ...
    */
	...
	public function actionSet()
	{
		...
		if (!isset($array['invoice_items'][0]))
		{
		//	dump('neni pole');
			$array['invoice_items'] = array($array['invoice_items']);
		}
		//dump($array['partner']);
		foreach ($array['invoice_items'] as $key1 => $onePricelist)
		{
	...
	public function actionGet()
	{
		...
		$xml = Array2XML::createXML('cl_invoice_advance_items', $tmpDataArr);
		echo $xml->saveXML();
		$this->terminate();		
	}
	...
	public function actionGetNew()
	{
		...
		$tmpData = $this->DataManager->findAllTotal()->where(array('cl_invoice_advance_items.cl_company_id' => $this->cl_company_id))->
								where('cl_invoice_advance_items.changed >= ? OR cl_invoice_advance_items.created >= ?', $this->sync_last, $this->sync_last);
		if (!empty($this->dataxml))
		{
			$tmpData = $tmpData->where('cl_invoice_advance_items.id NOT IN(?)', $this->dataxml);
		}
		
		$tmpData = $tmpData->select('cl_invoice_advance_items.*');
		
		$tmpDataArr = array();
		foreach($tmpData as $key => $one)
		{
			$cl_partners_book = $one->toArray();
			$tmpDataArr['cl_invoice_advance_items'][] = $cl_partners_book;
