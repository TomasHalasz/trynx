...
class SyncCashPresenter  extends \App\APIModule\Presenters\SyncPresenter{
    ...
    */
    ...
    public function actionSet()
	{
		...
		if (!isset($array['cl_cash'][0]))
		{
			$array['cl_cash'] = array($array['cl_cash']);
		}

		foreach ($array['cl_cash'] as $key1 => $oneSync)
		{
			...
			if (isset($oneSync['cis_poki']))
			    $tmpCis_record = $oneSync['cis_poki'];
            ...
            if ($oneSync['cl_invoice_arrived_id'] == "") {
                $oneSync['cl_invoice_arrived_id'] = NULL;
            }
            if ($oneSync['cl_invoice_id'] == "") {
...
*/
			...
			unset($oneSync['cis_poki']);
			unset($oneSync['locked']);
