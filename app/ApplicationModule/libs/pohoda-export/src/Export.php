...
class Export
{
    ...
    public function addBankTrans(BankTransaction $bankTransaction)
    {
        $this->bankTrans[] = $bankTransaction;
    }
	...
	public function exportToFile($exportId, $application, $fileName, $errorsNo, $note = '', $type = 'invoice')
	{

		$xml = $this->export($exportId, $application, $note, $type);
		$incomplete = '';
		if ($errorsNo > 0) {
			$incomplete = '_incomplete';
		}
		//$xml->asXML((is_null($this->exportFolder) ? dirname(__FILE__) : $this->exportFolder) . '/' . $fileName . '_lastId-' . $this->lastId . $incomplete . '.xml');
        $fileNameNew = (is_null($this->exportFolder) ? dirname(__FILE__) : $this->exportFolder) . '/' . $fileName . $incomplete . '.xml';
        $xml->asXML($fileNameNew);
        return $fileNameNew;
	}

	public function exportAsXml($exportId, $application, $note = '', $type = 'invoice')
	{
	...
	public function exportAsString($exportId, $application, $note = '', $type = 'invoice')
	{
		$xml = $this->export($exportId, $application, $note, $type);
		echo $xml->asXML();
	}
	...
	private function export($exportId, $application, $note = '', $type = 'invoice')
	{
        $typeNS = '';
        $type2 = '';
        $dataItems = [];
        if ($type == 'invoice'){
            $typeNS = Invoice::NS;
            $type2 = "inv";
            $dataItems = $this->invoices;
        }elseif ($type == 'bank_trans'){
            $typeNS = BankTransaction::NS;
            $dataItems = $this->bankTrans;
            $type2 = "bnk";
        }
