...
class Address implements IExport
{
	...
	public function exportFilter(SimpleXMLElement $xml)
	{
		...
		if($this->getIdentity()->hasId()) {
			...
			$ext->addChild('tpy:exSystemName', 'TRYNX', Export::NS_TYPE);
		} else {
			$adr = $this->getIdentity()->getAddress();
			$filter->addChild('ftr:company', htmlspecialchars($adr->getCompany(),ENT_XML1 | ENT_COMPAT, 'UTF-8'), Export::NS_FTR);
	...
	public function exportAddressXml(SimpleXMLElement $xml, \Pohoda\Object\Address $address, $type = self::ADDRESS)
	{
        ...
        $xmlAd->addAttribute('linkToAddress', 'true');
		if ($address->getCompany()) {
			$xmlAd->addChild('typ:company', htmlspecialchars($address->getCompany(),ENT_XML1 | ENT_COMPAT, 'UTF-8') );
		}

		if ($address->getName()) {
			$xmlAd->addChild('typ:name', htmlspecialchars($address->getName(),ENT_XML1 | ENT_COMPAT, 'UTF-8') );
		}
