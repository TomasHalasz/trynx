...
class Invoice
{
    ...
    const RECEIVED_TYPE = 'receivedInvoice'; //přijatá faktura
    const ADVANCE_TYPE = 'issuedAdvanceInvoice'; //zálohová faktura
	...
	private $varNum;
	private $originalDocument; //20.12.2020 TH
	private $date;
	private $dateTax;
	...
	private $priceNone;  //20.12.2020 TH
	private $priceLow;
	private $priceLowVAT;
	private $priceLowSum;
    private $priceLow3;
    private $priceLow3VAT;
    private $priceLow3Sum;
	private $priceHigh;
    ...
    /**
     * @param $type
     * issuedInvoice, issuedCorrectiveTax, receivedInvoice
     */
    ...
    public function setOriginalDocument($value)
    {
    ...
    /**
     * Set price without VAT (DPH - 10%)
     * @param float $pricelow3
     */
    public function setPricelow3($pricelow3)
    {
        $this->pricelow3 = $pricelow3;
    }

    /**
     * Set only VAT (DPH - 10%)
     * @param float $pricelow3VAT
     */
    public function setPricelow3VAT($pricelow3VAT)
    {
        $this->pricelow3VAT = $pricelow3VAT;
    }

    /**
     * Set price with VAT (DPH - 10%)
     * @param float $pricelow3Sum
     */
    public function setPricelow3Sum($pricelow3Sum)
    {
        $this->pricelow3Sum = $pricelow3Sum;
    }
		...
		try {
			$address = new Address(
				new \Pohoda\Object\Identity(
                    ...
                    (count($shippingAddress) > 0) ? new \Pohoda\Object\Address($shippingAddress) : null //pripadne dodaci adresa
				)
			);
	...
	private function exportHeader(SimpleXMLElement $header)
	{
        ...
        $header->addChild("inv:originalDocument", $this->originalDocument);
	...
	private function exportSummary(SimpleXMLElement $summary)
	{
        ...
        if (is_null($this->pricelow3) === false)
            $hc->addChild('typ:price3', $this->pricelow3, Export::NS_TYPE); //cena bez dph ve druhe snizene sazbe (10)
        if (is_null($this->pricelow3VAT) === false)
            $hc->addChild('typ:price3VAT', $this->pricelow3VAT, Export::NS_TYPE); //dph ve druhe snizene sazbe
        if (is_null($this->pricelow3Sum) === false)
            $hc->addChild('typ:price3Sum', $this->pricelow3Sum, Export::NS_TYPE); //s dph ve druhe snizene sazbe
		...
		$round->addChild('typ:priceRound', $this->priceRound, Export::NS_TYPE); //Celková suma zaokrouhleni
