...
class Ofx
{
    ...
    private function buildTransactions(SimpleXMLElement $transactions)
    {
        ...
        foreach ($transactions as $t) {
            ...
            $transaction->type = trim((string)$t->TRNTYPE);
            ...
            $transaction->uniqueId = trim((string)$t->FITID);
            $transaction->name = trim((string)$t->NAME);
            $transaction->memo = trim((string)$t->MEMO);
            $transaction->sic = $t->SIC;
            $transaction->checkNumber = $t->CHECKNUM;
            $transaction->accountNumber = $this->createBankAccountNumber($t->BANKACCTTO);
            $transaction->bankCode = $this->createBankCode($t->BANKACCTTO);
            $return[] = $transaction;
        }
     ...
     * @return mixed
     */
    private function createBankAccountNumber(SimpleXMLElement $xml)
    {
        $IBAN = (string)$xml->ACCTID;
        $account = rtrim(substr($IBAN,6));
     ...
     * @return mixed
     */
    private function createBankCode(SimpleXMLElement $xml)
    {
        $IBAN = (string)$xml->ACCTID;
        $account = substr($IBAN,3,4);
        return $account;
    }
    ...
    private function createAmountFromStr($amountString)
    {
        ...
        // Decimal mark style: 000.00000 or 0,000.00000
        //TH 26072021: update
        if (preg_match('/^(-|\+)?([\d,]+)(\.?)([\d]{5})$/', $amountString) === 1) {
            return (float)preg_replace(
                ['/([,]+)/', '/\.?([\d]{2})$/'],
                ['', '.$1'],
                $amountString
