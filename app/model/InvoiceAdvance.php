...
/**
 * InvoiceAdvance management.
 */
class InvoiceAdvanceManager extends Base
{
	const COLUMN_ID = 'id';
	public $tableName = 'cl_invoice_advance';
	...
	/**
	  ...
	  public function __construct(\Nette\Database\Context $db, UserManager $userManager, \Nette\Security\User $user, \Nette\Http\Session $session, \DatabaseAccessor $accessor,
					InvoiceAdvanceItemsManager $InvoiceAdvanceItemsManager, RatesVatManager $RatesVatManager,
                    InvoiceAdvancePaymentsManager $InvoiceAdvancePaymentsManager,CompaniesManager $CompaniesManager,
                    CashManager $CashManager, PairedDocsManager $pairedDocsManager)
	  {
    ...
    public function updateInvoiceProfit($id)
	{
		...
		$price_s			= $this->InvoiceAdvanceItemsManager->findBy(array('cl_invoice_advance_id' => $id))->sum('price_s * quantity');
		$price_e2			= $this->InvoiceAdvanceItemsManager->findBy(array('cl_invoice_advance_id' => $id))->sum('price_e2');
		$price_e2_vat		= $this->InvoiceAdvanceItemsManager->findBy(array('cl_invoice_advance_id' => $id))->sum('price_e2_vat');
	...
	public function updateInvoiceSum($id)
	{
	    ...
	    if ($tmpData->pdp == 1)
	    {
		    $recalcItems = $this->InvoiceAdvanceItemsManager->findBy(array('cl_invoice_advance_id' => $id));
		    foreach($recalcItems as $one)
		    {
				$data['price_e2_vat'] = 0;
				$one->update($data);
		    }
			/*$recalcItems = $this->InvoiceItemsBackManager->findBy(array('cl_invoice_advance_id' => $id));
			foreach($recalcItems as $one)
			{
				$data['price_e2_vat'] = 0;
				$one->update($data);
        ...
        /*$price_s_back		= $this->InvoiceItemsBackManager->findBy(array('cl_invoice_advance_id' => $id))->sum('price_s * quantity');
        $price_e2_back		= $this->InvoiceItemsBackManager->findBy(array('cl_invoice_advance_id' => $id))->sum('price_e2');
        $price_e2_vat_back	= $this->InvoiceItemsBackManager->findBy(array('cl_invoice_advance_id' => $id))->sum('price_e2_vat');*/
	    //}else{
		    //$price_e2_back		= 0;
		    //$price_e2_vat_back	= 0;
	    //}
	    ...
	    {
			$totalBase = $this->InvoiceAdvanceItemsManager->findBy(array('cl_invoice_advance_id' => $id, 'vat' => $one['rates']))->sum('price_e2');
		    //$totalBase = $totalBase - $this->InvoiceItemsBackManager->findBy(array('cl_invoice_advance_id' => $id, 'vat' => $one['rates']))->sum('price_e2');
	    ...
	    //odecteni zalohy
	    $price_payed = $this->InvoiceAdvancePaymentsManager->findAll()->where(array('cl_invoice_advance_id' => $id,
											       'pay_type' => 1));
	    ...
	    /*$parentData['price_payed'] = $this->InvoiceAdvancePaymentsManager->findAll()->where(array('cl_invoice_advance_id' => $id,
	    ...
	    {
		$parentData['pay_date'] = $this->InvoiceAdvancePaymentsManager->findAll()->where(array('cl_invoice_advance_id' => $id,
    ...
    public function paymentUpdate($id)
    {
        ...
        if (round($parentData['price_payed'],0) == round($tmpPrice,0)) {
            $parentData['pay_date'] = $this->InvoiceAdvancePaymentsManager->findAll()->where(array('cl_invoice_advance_id' => $id,
                'pay_type' => 0))->max('pay_date');
        }else{
            $parentData['pay_date'] = NULL;
        ...
        //get every cl_invoice_advance_payments and for each cl_payment_types make record in cl_cash
        foreach($tmpData->related('cl_invoice_advance_payments') as $key=>$one)
        {
            if (!is_null($one['cl_payment_types_id']) && $one->cl_payment_types->payment_type == 1){
                $tmpCashData = array();
                $tmpCashData['cl_invoice_advance_id']           = $tmpData->id;
                ...
                if (!is_null($tmpRetCashId)) {
                    ...
                    $this->PairedDocsManager->insertOrUpdate(['cl_invoice_advance_id' => $one->cl_invoice_advance_id, 'cl_cash_id' => $tmpRetCashId]);
                }
